...
AudioSystem::AudioSystem(int numChannels)
{
	...
	if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, numChannels, 2048) != 0) {
...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
	...
	if (availableChannel == -1) {
		for (auto& handle : mHandleMap) {
			if (handle.second.mSoundName == soundName) {
				availableChannel = i;
				SDL_Log("Stopping sound: %s", soundName.c_str());
	...
	if (availableChannel == -1 && !mHandleMap.empty()) {
		...
		SDL_Log("The oldest sound will be stopped: %s", soundName.c_str());
	...
	newHandleInfo.mChannel = availableChannel;
	newHandleInfo.mIsPaused = false;
	mHandleMap[mLastHandle] = newHandleInfo;
...
void AudioSystem::StopSound(SoundHandle sound)
{
	...
	if (it != mHandleMap.end()) {
		...
		mChannels[it->second.mChannel] = SoundHandle::Invalid;
		mHandleMap.erase(it);
	}
	SDL_Log("Error: Impossible to stop sound. Sound Not found.");
}
...
void AudioSystem::PauseSound(SoundHandle sound)
{
    ...
    //  mapa mHandleMap.
	for (auto& handle : mHandleMap) {
		if (handle.first == sound) {
			if (!handle.second.mIsPaused) {
	...
	SDL_Log("Error: Impossible to pause sound. Sound Not found.");
...
void AudioSystem::ResumeSound(SoundHandle sound)
{
	...
	SDL_Log("Error: Impossible to resume sound. Sound Not found.");
